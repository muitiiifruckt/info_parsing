...
2 вида поиска новостей:
1) Сбор всех новостей из источника и уже по ним искать самостоятелльно включения эмитентов в статьи
2) Целенаправленный поиск новостей по определнному эмитенту во всех источниках

_____________________________
Так как коммерсант добавил авторизацию по rss и другие могут подобное сделать
принято решение использовать поиск по 2 алгоритму. Для этого надо определить более менее универсальный метод сбора новостей по всем источникам и для каждого определить параметры разметки сайта. Поиск такой будет производиться с помощью эмулятора (plawright) - так как большинство работает с подгрузкой js - такой нельзя получить с помощью обычного requests.
